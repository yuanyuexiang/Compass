...
- **账号体系（审批制）**：`POST /api/auth/register` 创建待审批租户（`Tenant.status=pending, enabled=False`）+ tenant_admin；平台管理员（角色 `platform_admin`，种子 admin 即是）在「租户管理」页（`/api/admin`，`PlatformAdminDep`）审批/启停；platform_admin 账号统一挂靠**平台租户**（`Tenant.is_platform=True`，全库唯一，由 `core/db.py` 幂等迁移自动创建并把存量 admin 挪入）——平台租户不出现在租户列表、不可停用/删除（防自锁）、无画像订阅不参与匹配；tenant_admin 在「成员管理」页（`/api/tenant/users`）管本租户账号。登录失败 5 次锁 10 分钟（`core/ratelimit.py`，Redis，不可用则放行）；停用即时生效靠 `get_current_user` 里 60 秒 TTL 缓存的查库校验（`account_block_reason`）。密码强度统一走 `validate_password`。schema 变更用 `core/db.py` 的幂等 MIGRATIONS 列表（ADD COLUMN IF NOT EXISTS），Alembic 仍是待办。
...
- **流水线状态机**：`crawled → cleaned → attachments_parsed → ai_extracted → embedded → published`（+failed，+skipped 终态），发布后 fan-out 到各租户匹配。**自动 AI 提取有时效**（`AUTO_EXTRACT_MAX_AGE_HOURS`，默认 48h，`core/kv.py`）：公告**发布时间**（缺失回退采集时间）超时仍未提取的，由 `ai_backlog_tick`/`pipeline_sweep`/`ai_extract_task` 三处统一标 `skipped` 放弃（不烧 token、不计积压告警；按发布时间判定是为了整批拦下新源灌入的历史公告——采集时间新、发布时间旧），手动触发不受限。**不要物理删除 skipped 公告**：fingerprint 留着做采集去重，删了会被重新采回再烧一遍 token。**自动流水线原则：AI 能处理才采集**——`crawl_tick` 只在自动 LLM 时间窗内派发（夜间不采集、不攒积压；公告仍挂在源站，白天照常采到），且受背压（`auto_pipeline_backpressure`）：时效内待提取积压达 `AUTO_CRAWL_MAX_PENDING`（默认 50）或 LLM 连续失败达 `LLM_FAILURE_PAUSE_STREAK`（默认 5）次时暂停自动采集（不记 last_auto_crawl，压力解除下个 tick 即恢复）；LLM 连败期间 `ai_backlog_tick` 降为每 tick 单条探针（成功即清零计数自愈恢复满速）、`pipeline_sweep` 停派提取。健康告警只报真故障（等待超 24h 的 pending，排队 2 小时是限速消化的常态）。手动采集/手动触发不受背压与时间窗限制。历史性大积压用 `scripts/skip_extract_backlog.py` 一次性放弃（支持 `--keep-hours` 保留近期）。业务逻辑写成纯函数 `run_*(session,...)`，Celery 任务只是薄包装（`tasks/pipeline.py`）。
...
- **日志**：两条流分表存——**操作日志** `audit_logs`（谁做了什么：登录/注册/成员/画像/订阅/数据源/租户/LLM 配置约 30 处显式埋点）+ **运行日志** `system_events`（流水线事件：采集轮次/背压/放弃积压/LLM 故障/健康告警）。写入统一走 `core/audit.py` 的 `record_audit`/`record_event`（与业务同事务；**detail 严禁密码/API Key/JWT**，LLM 配置只记供应商名）；高频事件用 `acquire_cooldown` 防刷屏；Python logging 不入库（stdout 留排障）。查询：平台管理员 `GET /api/admin/audit-logs|system-events`，租户管理员 `GET /api/tenant/audit-logs`（强制本租户）；前端「日志」页按角色出 tab。保留 180/30 天（`core/kv.py`），Beat 每日 03:40 `logs_cleanup` 清理。新增埋点：路由里加一行 `record_audit`，前端 `ACTION_LABELS` 补中文（未收录的键原样展示不报错）。
