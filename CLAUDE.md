...
- `.github/workflows/deploy.yml`：master push 触发——构建 backend/frontend 镜像推仓库（REGISTRY 在 workflow env 里配置，默认 docker.io，阿里云 ACR 需改），SSH 到服务器执行 `deploy/deploy.sh`（拉镜像、compose up、幂等建表）。依赖 secrets：DEPLOY_HOST / DEPLOY_USER / DEPLOY_SSH_KEY / REGISTRY_USERNAME / REGISTRY_PASSWORD。
