...
- **流水线状态机**：`crawled → cleaned → attachments_parsed → ai_extracted → embedded → published`（+failed，+skipped 终态），发布后 fan-out 到各租户匹配。**自动 AI 提取有时效**（`AUTO_EXTRACT_MAX_AGE_HOURS`，默认 48h，`core/kv.py`）：公告**发布时间**（缺失回退采集时间）超时仍未提取的，由 `ai_backlog_tick`/`pipeline_sweep`/`ai_extract_task` 三处统一标 `skipped` 放弃（不烧 token、不计积压告警；按发布时间判定是为了整批拦下新源灌入的历史公告——采集时间新、发布时间旧），手动触发不受限。**不要物理删除 skipped 公告**：fingerprint 留着做采集去重，删了会被重新采回再烧一遍 token。**自动流水线有背压**（`auto_pipeline_backpressure`）：时效内待提取积压达 `AUTO_CRAWL_MAX_PENDING`（默认 300）或 LLM 连续失败达 `LLM_FAILURE_PAUSE_STREAK`（默认 5）次时，`crawl_tick` 暂停自动采集（不记 last_auto_crawl，压力解除下个 tick 即恢复）；LLM 连败期间 `ai_backlog_tick` 降为每 tick 单条探针（成功即清零计数自愈恢复满速）、`pipeline_sweep` 停派提取。手动采集/手动触发不受背压限制。历史性大积压用 `scripts/skip_extract_backlog.py` 一次性放弃（支持 `--keep-hours` 保留近期）。业务逻辑写成纯函数 `run_*(session,...)`，Celery 任务只是薄包装（`tasks/pipeline.py`）。
